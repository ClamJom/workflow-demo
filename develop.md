...
5. 节点在创建后，应当在`com.example.demoworkflow.utils.workflow.nodes.NodeType`中写入一个新的节点类型，并在静态方法中将新建的节点类放入`nodeClazzMap`。新建结果类型后，其相应的结果类型也应当在新建的节点初始化函数中使用`setNodeType`方法指定。
...
2. 前端需要提供相应的配置编辑方法，如`Boolean`对应开关、`List`与`Map`对应可拓展表、`Condition`对应一个含有左值，右值，运算符的条件编辑器、`Number`与`String`对应相应的输入框，其中`Number`应当含有高级选项以配置取值区间与精度和浮点除数的选项。

## Vue-Flow节点

1. 根据后端定义的节点类型，前端应当定义相应的Vue-flow节点。其中`StartNode`与`EndNode`应当显示为圆形节点，分别为绿色与橙色，其它节点均为白色矩形。普通的工作节点应当至少含有一个Handler（即输出端）可供多个其它节点接入，但条件节点应根据条件数量设置Handler数量。节点不允许自指（从节点出发创建边指向自己）。
