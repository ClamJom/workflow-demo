...
这里的“全局”是相对当前工作流而言的，该变量池向工作流中的所有节点共享，在整个生命周期中都可以通过工作流Token与变量名称访问。由于涉及到多线程访问，需要适当的加锁进行处理。在节点后置Hook中提到，节点的变量池会与全局变量池合并，一般的，节点变量池会在变量名称前加上节点id用于区分节点所属的变量以方便后续的节点访问和注入变量，因此不需要过渡关注变量冲突问题。
